		...
		if(cart) {
			...
			jQuery.each(cart.items,function(i,item){
				...
				if(item.image != null)
					...
					tr.find('.item-image').html("<img src='//theme.hstatic.net/1000370106/1000775487/14/no_image.jpg?v=1651'>");
			...
			if(cart.item_count == 0){				
				...
				jQuery('#cart-view').html('<tr><td>YOUR CART IS EMPTY</td></tr>');
				jQuery('#cartform').hide();
			}
			else{			
...
function clone_item(product,i){
	...
	if ( product.image == null ) {
		item_product.find('img').attr('src','//theme.hstatic.net/1000370106/1000775487/14/no_image.jpg?v=1651').attr('alt', product.url);
	} else {
...
jQuery(window).scroll(function() {
	...
	if (jQuery(window).width() < 768) {
		...
		if (scroll < 320) {
			$(".main-headerxx").removeClass("scroll-menu");	
		} else {
			$(".main-headerxx").addClass("scroll-menu");		
		}
	} else {
		var height_header =	$('.main-header').height();
		if( jQuery(window).scrollTop() >= 50 ) {			
			jQuery('.main-headerxx').addClass('affix-mobile');
		}	else {
			jQuery('.main-headerxx').removeClass('affix-mobile');
		}
	}
});
...
jQuery(document).ready(function(){
	...
	jQuery('#collection-slidex').owlCarousel({
		items:4,
		nav:true,
		dots:false,
		responsive:{
	...
	jQuery('#fifth-banner-slide').owlCarousel({
		items:4,
		nav:true,
		dots:false,
		responsive:{
...
jQuery(document).ready(function(){
	...
	$('#popup-size').on('click',function(e){
		e.preventDefault();
		$('#myModal-1').toggleClass('showx');
	})
